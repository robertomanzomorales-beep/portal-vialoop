...
async function tokenFromRequest(request: Request) {
  ...
  if (typeof token !== "string" || !token.trim()) {
    throw new Error("Flow no envió el token de confirmación.");
  }
...
export async function POST(request: Request) {
  try {
    ...
    console.info("Confirmación Flow de cobro manual procesada:", result);
    ...
    console.error(
      "Error al procesar la confirmación Flow de un cobro manual:",
      error,
    );
